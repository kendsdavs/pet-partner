...
const Pets = React.createClass({
    ...
    componentDidMount() {
        ...
        data.list("pets").then(pets => this.setState({filtered: pets.docs, pets: pets.docs}))
    },
    filter(e) {
        this.setState({
            filtered: filter(pet => pet.name.indexOf(e.target.value) === 0, this.state.pets)
        })
    },
    render() {
        const list = pet => <ListGroupItem key={pet._id}>
